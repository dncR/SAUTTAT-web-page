(() => {
  const SAUTTAT = (window.SAUTTAT = window.SAUTTAT || {});
  if (SAUTTAT.__globalScriptsInitialized) return;
  SAUTTAT.__globalScriptsInitialized = true;
  ...
  const startSharedUI = () => {
    if (SAUTTAT.sharedReady) return SAUTTAT.sharedReady;
    SAUTTAT.sharedReady = initSharedUI();
    return SAUTTAT.sharedReady;
  };

  SAUTTAT.initSharedUI = initSharedUI;
  SAUTTAT.waitForSharedUI = () => startSharedUI();
